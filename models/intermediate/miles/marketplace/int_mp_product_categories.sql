select
    ...
    , awc.BRAND_ID
    , ( {{ get_product_url('acp.PRODUCT_ID', 'awc.BRAND_ID') }} ) as PRODUCT_URL
    , awc.CATEGORY_ID
    , DISPLAY_NAME as CATEGORY_NAME
    , PARENT_CATEGORIES
from
    {{ ref('int_active_categories_joined_with_category_products') }} as awc
    join {{ ref('mp_active_products') }} as acp on awc.PRODUCT_ID = acp.PRODUCT_ID
where
    ...
    or (
        ...
        and not exists(
            select
                1
            from
                {{ ref('int_mp_product_slices_categories') }} as psc
            where
