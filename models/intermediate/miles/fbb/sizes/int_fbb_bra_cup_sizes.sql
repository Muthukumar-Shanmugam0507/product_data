select distinct
    sc.MF_SIZE_ID as MF_SIZE_ID
    , sc.MF_DISPLAY_SIZE as MF_DISPLAY_SIZE
    , sc.GROUP_ID as GROUP_ID_3,
    case 
        when
            cl.GROUP_ID = 2
            or (
                cl.GROUP_ID = 0
                and pr.CLASSIFICATION_ID in (42, 44, 45)
                and pr.MF_DEPARTMENT in (17, 33)
            )
        then
            split(sc.SPLIT_SIZE_2, '/')
        else
            null
    end as bra_cup_size
from
